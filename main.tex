\documentclass{article}
\usepackage[utf8]{inputenc}
\usepackage{graphicx}
\usepackage{subcaption}
\usepackage{float}
\usepackage{hyperref}
\usepackage{amsmath}
\newcommand{\mypapersize}{A4}
\title{Pattern Recognition Cheat Sheet}
\author{CCE 2020}
\begin{document}
\maketitle
\tableofcontents

%%%%%%%%%%%% Dimension Reduction Section %%%%%%%%%%%%%%%%%%%%
\section{Dimensionality Reduction}
    We reduce dimensionality to Compress Data, 
    Reduce Noise, Visualization and Apply Euclidean geometry comfortably.
    \subsection{PCA}
        \input{BodyComponents/pca.tex}
    \subsection{LDA}
        \input{BodyComponents/lda.tex}

%%%%%%%%%%%% Clustering Section %%%%%%%%%%%%%%%%%%%%
\section{Unsupervised Learning: Clustering}
    \subsection{K-means Clustering}
        \input{BodyComponents/kmeans.tex}
    \subsection{Spectral Clustering }
        \input{BodyComponents/spectral.tex}
    \subsection{Clustering Evaluation}
        \input{BodyComponents/cluster_eval.tex}
    \newpage

%%%%%%%%%%%% Classification Section %%%%%%%%%%%%
\section{Supervised Learning: Classification}
    Classification Pipeline includes: Input (training set), Learning a model and Evaluation of the quality of the classifier by asking it to predict labels for a new set of objects that it
    has never seen before (test set) then compare the true labels of these objects to the ones predicted by the classifier.
    \subsection{KNN: K- Nearest Neighbour Classifier}
        \input{BodyComponents/knn.tex}
    \subsection{Full Bayes Classifier}
        \input{BodyComponents/full_bayes.tex}
    \subsection{Naive Bayes Classifier}
        \input{BodyComponents/naive_bayes.tex}
    \newpage
    \subsection{Decision Tree Classifier}
        \input{BodyComponents/decision_tree.tex}
    \newpage
    \subsection{Ensemble Methods}
        \input{BodyComponents/ensemble.tex}

%%%%%%%%%%%% Other %%%%%%%%%%%%%
\section{Miscellaneous Rules and Linear Algbera}
\input{BodyComponents/other.tex}
\end{document}
